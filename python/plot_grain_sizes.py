...
for i in tqdm(range(len(args.files)), ncols = 80):
    ...
    data = [j[1:] for j in all_data[i]] # the remaining columns are the sizes for each grain
    ...
    for j in range(len(data[0])): # each 'row' of data has the number of grains being plotted
        ...
        color_index += 1
    plot_sets.append(plt.plot([], [], 'k-' + marker_styles[marker_index], label = '{}'.format(args.names[i]), markevery = mark_every, lw = 2))
    marker_index += 1
