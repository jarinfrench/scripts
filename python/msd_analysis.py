...
parser = argparse.ArgumentParser(usage = '%(prog)s [-h] base [options]', description = "Create MSD plots of all temperatures and simulations of a specific misorientation.")
parser.add_argument('base', help = "basename of the saved files")
parser.add_argument('-T', type = int, nargs = 2, help  = "Minimum and maximum temperatures to plot MSD of (default: all temperatures)", default = [0,100000])
parser.add_argument('--dt', type = float, help = "The timestep used in the simulations (used to convert from timesteps to time)", default = 0.002)
parser.add_argument('-n', type = int, nargs = '+', help = "\"Dir\" numbers to plot (default: all)")
parser.add_argument('-i', action = 'store_true', help = "Interactive plotting (WARNING: uses a lot of memory!)")
parser.add_argument('-2', dest = "two", action = 'store_true', help = "Create the 2D MSD plot (xy, xz, and yz)")
parser.add_argument('-3', dest = "three", action = 'store_true', help = "Create the 3D MSD plot (xyz)")
...
if not args.n:
    dirs = list(set(os.path.basename(os.path.dirname(i)) for i in glob.glob('*/*/dir_*/')))
...
for T in Ts:
    for n in args.n:
        if not os.path.isdir(f"T{T}/large_r/dir_final_{n}"):
            continue
        msd = np.recfromcsv(f"T{T}/large_r/dir_final_{n}/MSD_U.dat", delimiter = " ", dtype = [('timestep', int), ('msd_x', float), ('msd_y', float), ('msd_z', float),('msd_xyz',float)], skip_header = 2, names = None)
        ...
        if args.i:
            ...
            for handle in lgnd.legendHandles:
                handle._sizes = [30]
            figs[(T,n)].savefig(f"{args.base}_T{T}_dir{n}_msd.png", bbox_inches='tight')
