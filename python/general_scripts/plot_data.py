...
parser.add_argument('--xlim', nargs = 2, type = float, help = "Limits for the x axis")
parser.add_argument('--ylim', nargs = 2, type = float, help = "Limits for the y axis")
...
parser.add_argument('--median', action = 'store_true', help = "Print the median of the data")
parser.add_argument('--mode', action = 'store_true', help = "Print the mode of the data")
parser.add_argument('-n', '--norm', choices = ['x','y','xy'], help = "Normalize the x, y, or both by the respective maximums")
parser.add_argument('--normalize-by', type = float, help = "Normalize the y data by dividing by this factor")
...
parser.add_argument('--markersize', type = float, default = 0.5, help = "The size of the plotted points (default: 0.5)")
parser.add_argument('-s','--save', help = "Saves the plot with the specified file name")
parser.add_argument('--publish', action = 'store_true', help = "Save the figure as a publication-ready plot (uses the 'save' option file name)")
...
for i in range(len(args.files)):
    ...
    reader = csv.reader(fin, delimiter = '\t' if args.delimiter == r"\t" else args.delimiter)
    ...
    if args.publish:
        ...
        if args.plot_type == 'l':
            plot_style[i] += '-'
        plt.plot(x_data, y_data, plot_style[i], label=args.labels[i], markersize = args.markersize)
...
if args.median:
    for i,item in enumerate(data_medians):
        ...
        plt.axhline(y=item, color = plot_style[i][0], linestyle='--', label=args.labels[i] + " median")
if args.mode:
...
if not args.no_legend:
    if args.publish:
        ...
        plt.legend(loc='best', ncol = args.legend_columns, markerscale = 5/args.markersize)
if args.save:
