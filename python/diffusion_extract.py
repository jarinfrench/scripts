...
for T in Ts:
    filenames = [f"T{T}/large_r/dir_{i}/diffusion_results.txt" for i in [1, 2, 3]]
    diffusions = {}
        ...
        with open(file, "r") as f:
            for line in f:
                ...
                if not line.split():
                    continue
                if ll[0] == "MSD":
                    key1 = getSubStr(ll[2], "_", ".")
                if ll[0] == "GB" or ll[0] == "Bulk":
                    key2 = ll[0]  # GB or Bulk
                    key3 = ll[3]  # x or y or z or xy or xz or yz or xyz
                    val = float(ll[5])
    ...
    for atom in diffusions.keys():
        for diff_type in diffusions[atom].keys():
            for direction in diffusions[atom][diff_type].keys():
                ...
                if T == 1050:  # skip if T1050 doesn't exist for whatever reason
