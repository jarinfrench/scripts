...
def func3D(data, z0, x1, y1, x2, y2, x3, y3, xy, xy2, x2y):
    ...
    return x3 * x**3 + x2 * x**2 + x1 * x + z0 + y3 * y**3 + y2 * y**2 + y1 * y + xy * x * y + xy2 * x * y * y + x2y * x * x * y
...
def SurfacePlot(func, data, fittedParameters):
    ...
    # R = Z
    # R = np.where(Y + 250000 * X * X + 13886.3636363636 * X < 1872.5, Z, np.nan) # for U_Mo_Xe_ternary_eam - UXe
    #R = np.where(Y - 5994.006 * X * X + 2261.072 * X <= 2011.888, Z, np.nan) # for U_Mo_Xe_ternary_eam - UMo
    ...
    # NOTE: set vmin, vmax according to the min/max of the lattice parameter, giving a bit of space
    # Then, set_zlim is just 0.01 lower/higher (respectively)
    surf = axes.plot_surface(X, Y, Z, alpha = 0.75, cmap=cm.coolwarm, linewidth=0, antialiased=False, vmin = min(z_data) * 0.99, vmax = max(z_data) * 1.01, zorder = 2)
...
parser = argparse.ArgumentParser(usage = '%(prog)s [-h] file a0 runs(3)', description = "Perform a fit to the lattice parameter as a function of temperature and concentration")
parser.add_argument("file", help = "The file containing the temperature and lattice parameters")
parser.add_argument("a0", type = float, help = "The estimated lattice_parameter")
parser.add_argument("runs", type = int, nargs = "?", default = 3, help = "The number of runs per temperature. Default: 3")
parser.add_argument("-T", "--temperature-only", action = 'store_true', help = "Calculate a fit using temperature only")
parser.add_argument("-d", "--delimiter", default = ' ', help = "Specify the delimiter for reading the data file. Default: ' '")
...
with open(args.file) as f:
    reader = csv.reader(f, delimiter=args.delimiter, quotechar='"') # change the delimiter as needed
    for row in reader:
        if len(row) < 5:
            print(f"Only found {len(row)} elements: did you forget to indicate the structure?")
            exit(1)
        data.append(tuple(row))

if args.temperature_only:
    # data format:    Temp,        ax,          ay,          az,                           solid(1) or liquid(0)
    data = np.array([(float(i[0]), float(i[1]), float(i[2]), float(i[3])) for i in data if int(i[4]) == 1])
else:
...
for chunk in mine.chunker(data, args.runs):
    ...
    if args.temperature_only:
        avg_data.append(np.append(d[0:4], [np.mean(chunk[:,1:4]), stats.stdev(mine.flatten(chunk[:,1:4]))/np.sqrt(args.runs * 3)]))
    else:
...
if args.temperature_only:
    ...
    print("z0 = {}\nx1 = {}\ny1 = {}\nx2 = {}\ny2 = {}\nx3 = {}\ny3 = {}\nxy = {}\nxy2 = {}\nx2y = {}".format(*params))
    SurfacePlot(func3D, np.array([[i,j,k] for i,j,k in zip(x_data, y_data, z_data)]), params)
