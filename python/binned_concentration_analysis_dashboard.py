...
center = [
    (x_bounds[1] - x_bounds[0]) / 2,
    (y_bounds[1] - y_bounds[0]) / 2,
...
n_imps = (
    len(
        pd.read_csv(
            files[0], sep=" ", names=tuple(ddtype.keys()), dtype=ddtype, skiprows=9
        ).type.unique()
    )
    - 1
)
...
raw_data = pd.concat(
    (
        pd.read_csv(
            file, sep=" ", names=tuple(ddtype.keys()), dtype=ddtype, skiprows=9
        ).assign(timestep=int(file.split(".")[0]))
        for file in files
    ),
    ignore_index=True,
).assign(
    radius=lambda x: np.sqrt(
        np.sum((x.iloc[:, 2:4] - np.array(center[0:2])) ** 2, axis=1)
    )
)
...
area_data = pd.read_csv(
    ...
    skiprows=1,
).assign(timestep=lambda x: x.time / 0.002, radius=lambda x: np.sqrt(x.area2 / np.pi))
...
for i in timesteps:
    ...
    snapshot_figs_all[i].update_layout(
        autosize=False, width=600, height=600, margin=dict(l=5, r=10, b=50, t=50, pad=4)
    )
    snapshot_figs_all[i].update_xaxes(
        showgrid=False, showline=False, showticklabels=False, zeroline=False
    )
    snapshot_figs_all[i].update_yaxes(
        showgrid=False, showline=False, showticklabels=False, zeroline=False
    )
    snapshot_figs_no_U[i].update_layout(
        autosize=False, width=600, height=600, margin=dict(l=5, r=10, b=50, t=50, pad=4)
    )
    snapshot_figs_no_U[i].update_xaxes(
        showgrid=False, showline=False, showticklabels=False, zeroline=False
    )
    snapshot_figs_no_U[i].update_yaxes(
        showgrid=False, showline=False, showticklabels=False, zeroline=False
    )
    selected_data_no_U = raw_data[(raw_data.timestep == i) & (raw_data.type != 1)]
    selected_data_all = raw_data[
        (raw_data.timestep == i)
        & (((raw_data.type == 1) & ((raw_data.id % 3) == 0)) | (raw_data.type != 1))
    ]
    snapshot_figs_all[i].add_trace(
        go.Scatter(
            x=selected_data_all["x"],
            y=selected_data_all["y"],
            mode="markers",
            marker_color=[cmap[i - 1] for i in selected_data_all.type],
            marker_size=[3 if i == 1 else 6 for i in selected_data_all.type],
        )
    )
    snapshot_figs_no_U[i].add_trace(
        go.Scatter(
            x=selected_data_no_U["x"],
            y=selected_data_no_U["y"],
            mode="markers",
            marker_color=[cmap[i - 1] for i in selected_data_no_U.type],
            marker_size=[3 if i == 1 else 6 for i in selected_data_no_U.type],
        )
    )

default_fig = go.Figure()
default_fig.update_layout(
    autosize=False, width=1000, height=600, margin=dict(l=5, r=10, b=50, t=50, pad=4)
)
...
default_fig2.update_layout(
    autosize=False, width=600, height=600, margin=dict(l=5, r=10, b=50, t=50, pad=4)
)
default_fig2.update_xaxes(
    showgrid=False, showline=False, showticklabels=False, zeroline=False
)
default_fig2.update_yaxes(
    showgrid=False, showline=False, showticklabels=False, zeroline=False
)
default_fig3 = go.Figure()
default_fig3.update_layout(
    autosize=False, width=600, height=600, margin=dict(l=5, r=10, b=50, t=50, pad=4)
)
...
app.layout = html.Div(
    [
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        dcc.Input(id="ring-number-input", type="number", min=1, step=1),
                        html.Label("Max value: ", id="max-ring-num-label"),
                    ],
                    style={"width": "25%", "float": "left", "padding-left": "10px"},
                ),
                html.Div(
                    [
                        html.Label("Impurity type:"),
                        dcc.RadioItems(
                            [
                                {"label": i, "value": f"N_imp{i}"}
                                for i in range(1, n_imps + 1)
                            ],
                            id="impurity-radio",
                    ...
                    hidden=True if n_imps == 1 else False,
                ),
            ],
        ...
        html.Div(
            [
                html.Div(
                    [
                        dcc.Loading(
                            id="concentration-graph-loading",
                            children=[
                                html.Div(
                                    dcc.Graph(
                                        figure=default_fig, id="concentration-graph"
                                    )
                                )
                            ],
                            type="circle",
                ...
                html.Div(
                    [
                        dcc.Graph(figure=default_fig2, id="snapshot-graph"),
                        dcc.Checklist(
                            [{"label": "Hide U atoms", "value": True}],
                            id="hide-U-checklist",
                        ),
                    ],
                    style={
        ...
        # html.Div(
        #     [
        #         dcc.Graph(figure=default_fig3, id="c-vs-distance-graph"),
        #         dcc.Slider(
        #             id="timestep-slider",
        #             value=0,
        #             min=0,
        #             max=area_data["time"].max(),
        #             step=area_data["time"].diff().iloc[0],
        #             included=False,
...
@app.callback(Output("area-data", "data"), Input("ring-width-input", "value"))
def update_area_data_rings(ring_width):
    if not ring_width:
        return None
    d = deepcopy(area_data)
    n_rings = int(
        np.ceil(max([x_bounds[1] - center[0], y_bounds[1] - center[1]]) / ring_width)
    )
    d = d.assign(
...
def update_data(ring_width):
    ...
    tmp_data = raw_data.assign(
        ring_num=lambda x: np.digitize(x.radius / ring_width, np.array(range(n_rings)))
    )
    multi_index = pd.MultiIndex.from_product(
        [timesteps, range(1, n_rings + 1)], names=["timestep", "ring_num"]
    )
    concentration_data = pd.DataFrame(
        convertData(tmp_data, multi_index).values(),
        multi_index,
        columns=["N_ring", "N_imp1", "N_imp2"],
    )
    concentration_figs = {}
    for i in range(1, ring_width + 1):
        ...
        for imp_type in ["N_imp1", "N_imp2"]:
            y = [
                concentration_data.loc[i][imp_type]
                / concentration_data.loc[i]["N_ring"]
                * 100
                for i in selected_keys
            ]
            concentration_figs[i][imp_type] = go.Figure()
            concentration_figs[i][imp_type].update_layout(
                autosize=False,
                width=1000,
                height=600,
                margin=dict(l=5, r=10, b=50, t=50, pad=4),
            )
            concentration_figs[i][imp_type].update_xaxes(
                title="Time (ps)", range=[0, 8000]
            )
            concentration_figs[i][imp_type].update_yaxes(
                title="Impurity concentration (percent)", range=[0, yrange[ctotal]]
            )
            concentration_figs[i][imp_type].add_trace(
                go.Scatter(x=x, y=y, mode="lines+markers")
            )

    return (
        concentration_data.reset_index().to_json(orient="split"),
        n_rings,
        f"Max value: {n_rings}",
        concentration_figs,
    )
...
def update_concentration_figure(c_data, a_data, ring_num, imp_type, c_figs):
    ...
    # selected_keys = [
    #     key for key in concentration_data.index.values if key[1] == ring_num
    # ]
    ...
    if np.any(selected_times):
        ...
        fig.add_vrect(
            ...
            fillcolor="red",
            opacity=0.2,
            annotation_text="GB within specified ring",
...
def update_snapshot(hoverData, ring_num, ring_width, hide_U):
    ...
    fig.add_trace(
        go.Scatter(
            x=small_circle_x, y=small_circle_y, line=dict(color="Gray", dash="dash")
        )
    )
    if not ring_num == n_rings:
        fig.add_trace(
            go.Scatter(
                x=large_circle_x, y=large_circle_y, line=dict(color="Gray", dash="dash")
            )
        )
    fig.update_layout(
        showlegend=False,
        ...
        title={
            "text": f"Snapshot at t = {timestep * 0.002}ps",
            "x": 0.5,
            "y": 0.9,
            "xanchor": "center",
            "yanchor": "top",
...
# @app.callback(
#     Output("c-vs-distance-graph", "figure"),
#     Input("ring-width-input", "value"),
