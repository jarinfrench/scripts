...
def calculate_MSDs(frame, data):
    ...
    msd_all = np.sum(displacement_magnitudes_all ** 2) / len(displacement_magnitudes_all)
    l = len(displacement_magnitudes)
    tmp = np.array([i[0] for i in displacement_magnitudes])
    msd_x = np.sum(np.square(tmp)) / l
    tmp = np.array([i[1] for i in displacement_magnitudes])
    msd_y = np.sum(np.square(tmp)) / l
    tmp = np.array([i[2] for i in displacement_magnitudes])
    msd_z = np.sum(np.square(tmp)) / l
...
export_file(pipeline, "MSD_U.dat", format = "txt/attr", columns = ["Timestep", "MSD X", "MSD Y", "MSD Z", "MSD"], multiple_frames = True)
