...
int main(int argc, char **argv)
{
  ...
  double x, y, z; // atom charge, and x, y and z positions
  double r_grain;
  ...
  fout << "These Cu coordinates have been annealed: [ID type x y z]\n\n";
  ...
  fout << "1   atom types\n"; // WARNING: This is assuming we are working with Cu only!
  ...
  fin.ignore();
  getline(fin, str); // Gets the line "ITEM: ATOMS id type x y z c_pe_layer1"
  fout << "\nAtoms\n\n"; // Now we can start reading the atoms

  while (fin >> id >> type >> x >> y >> z >> str)
  {
    if (fin.fail())
    {
      cout << "Read error.\n";
